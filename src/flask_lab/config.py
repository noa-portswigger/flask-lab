...
def load_config(config_path: str) -> Config:
    ...
    if db_type not in ("sqlite", "rds"):
        raise ValueError(f"Invalid database type: '{db_type}'. Must be either 'sqlite' or 'rds'")

    # Parse sqlite config if type is sqlite
    if db_type == "sqlite":
        sqlite_data = db_data.get("sqlite", {})
        sqlite_config = SqliteConfig(uri=sqlite_data.get("uri", "sqlite:////tmp/todo.db"))
        rds_config = None
    else:
        ...
        if missing_fields:
            raise ValueError(f"Missing required RDS configuration fields: {', '.join(missing_fields)}")
    ...
    database_config = DatabaseConfig(type=db_type, sqlite=sqlite_config, rds=rds_config)
